...
- **`instance.annotationManager`** (also `instance.Core.annotationManager`): `getAnnotationsList()`, `getAnnotationById(id)`, `addAnnotations(list, { imported?, force? })`, `updateAnnotation(id, patch)`, `deleteAnnotations(idsOrAnnotations, { imported?, force? })`, `undo()`, `redo()`, `importAnnotations(xfdf)` (merges by XFDF `name`, flagged `imported`), `exportAnnotations({ annotList?, useDisplayAuthor? })` (bare fragments with `annotList`, full document otherwise), `exportAnnotationsLegacyJson()`, `setCurrentUser(name, id?)`, `setReadOnly(bool)`, `setCanEditOthers(bool)`, `canEdit(annotation)`, `addEventListener('annotationChanged', ({ annotations, action, imported }) => …)`.
- **`instance.print()`**: prints the exported document through the browser's PDF viewer.
- Legacy facades kept for drop-in compatibility: `instance.UI.openElements([...])`, `closeElements`, `setActiveLeftPanel`, `fitWidth`, `fitPage`, `setToolMode`; `instance.Core.documentViewer.getDocument().getFileData()`, `addEventListener(event, cb)`.
